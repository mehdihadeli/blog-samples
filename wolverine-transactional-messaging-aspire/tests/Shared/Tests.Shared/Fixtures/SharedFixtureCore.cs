...
/// <summary>
/// Common container + broker-assertion infrastructure shared by every test
/// fixture (service-level fixtures booted via <c>WebApplicationFactory&lt;T&gt;</c>
/// and isolated building-block fixtures that boot their own host via
/// <c>WebApplication.CreateBuilder</c>). Holds the Testcontainers instances, the
/// <c>TrackActivity</c>-based consume/send/publish assertions, and the per-test
/// reset logic — everything except HOW the host is created, which stays in each
/// derived fixture.
/// </summary>
public abstract class SharedFixtureCore(
...
) : IAsyncLifetime
{
    ...
    /// <summary>The application service provider (from whichever host the derived fixture boots).</summary>
    ...
    )
    {
        ...
        var session = ServiceProvider.TrackActivity().Timeout(TestTimeout);
