...
`ECommerce.BuildingBlocks.TestHost` is the shared ASP.NET entry point. It is referenced as a project by each building-block test project and hosted in-process via `WebApplicationFactory<Program>` — it is **not** started as a standalone process.

The RabbitMQ building-block test fixture ([RabbitMqBuildingBlocksSharedFixture.cs](https://github.com/mehdihadeli/blog-samples/blob/main/wolverine-transactional-messaging-aspire/tests/BuildingBlocks/ECommerce.BuildingBlocks.RabbitMQ.IntegrationTests/RabbitMqBuildingBlocksSharedFixture.cs)) starts a real RabbitMQ container and overrides configuration so the host uses the RabbitMQ transport with manual topology and no durable storage. The host's [Program.cs](https://github.com/mehdihadeli/blog-samples/blob/main/wolverine-transactional-messaging-aspire/tests/BuildingBlocks/ECommerce.BuildingBlocks.TestHost/Program.cs) then wires `AddWolverineRabbitMq(..., configure: rabbitMq => rabbitMq.ConfigureTestRabbitMqTopology())`.
