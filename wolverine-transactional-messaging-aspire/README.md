...
In addition to the service-level tests, `tests/BuildingBlocks` hosts isolated Wolverine building-block tests. These tests verify publish/consume round-trips and broker topology without involving the `Catalogs` or `Orders` APIs.

There is **no shared test-host project**. Each building block is its own project and needs its own host wiring, so every building-block test fixture builds its own host with `WebApplication.CreateBuilder` — mirroring how Wolverine's own transport tests boot hosts right inside the fixture (e.g. [send_by_topics.cs](https://github.com/JasperFx/wolverine/blob/main/src/Transports/RabbitMQ/Wolverine.RabbitMQ.Tests/send_by_topics.cs)). The base fixture ([BuildingBlocksSharedFixture.cs](https://github.com/mehdihadeli/blog-samples/blob/main/wolverine-transactional-messaging-aspire/tests/Shared/Tests.Shared/Fixtures/BuildingBlocksSharedFixture.cs)) owns container + host lifecycle and exposes a `ConfigureBuilder(WebApplicationBuilder)` hook plus an optional `ConfigurePipeline(WebApplication)` middleware hook; the test messages, handlers and topology live inside each test project.

The RabbitMQ building-block test fixture ([RabbitMqBuildingBlocksSharedFixture.cs](https://github.com/mehdihadeli/blog-samples/blob/main/wolverine-transactional-messaging-aspire/tests/BuildingBlocks/ECommerce.BuildingBlocks.RabbitMQ.IntegrationTests/RabbitMqBuildingBlocksSharedFixture.cs)) starts a real RabbitMQ container, builds its own host via `WebApplication.CreateBuilder`, and wires `AddWolverineRabbitMq(..., configure: rabbitMq => rabbitMq.ConfigureTestRabbitMqTopology())` with manual topology and no durable storage.

The tests cover:

- Round-trips through topic exchange, direct queue, and conventional routing ([WolverineRabbitMqPublishTests.cs](https://github.com/mehdihadeli/blog-samples/blob/main/wolverine-transactional-messaging-aspire/tests/BuildingBlocks/ECommerce.BuildingBlocks.RabbitMQ.IntegrationTests/WolverineRabbitMq/WolverineRabbitMqPublishTests.cs))
- Broker topology verification through the RabbitMQ management API ([WolverineRabbitMqTopologyTests.cs](https://github.com/mehdihadeli/blog-samples/blob/main/wolverine-transactional-messaging-aspire/tests/BuildingBlocks/ECommerce.BuildingBlocks.RabbitMQ.IntegrationTests/WolverineRabbitMq/WolverineRabbitMqTopologyTests.cs))

The RabbitMQ reset is narrow on purpose: it purges only the contents of each queue through the management API (`DELETE /api/queues/{vhost}/{name}/contents`). Exchanges, queues, and bindings survive, so Wolverine's `AutoProvision` topology stays intact and the shared host never restarts between tests.

The Kafka building-block fixture ([KafkaBuildingBlocksSharedFixture.cs](https://github.com/mehdihadeli/blog-samples/blob/main/wolverine-transactional-messaging-aspire/tests/BuildingBlocks/ECommerce.BuildingBlocks.Kafka.IntegrationTests/KafkaBuildingBlocksSharedFixture.cs)) looks the same, except it cannot wipe the broker between tests. Kafka has no queue/exchange split: a topic is the queue and the binding in one, so `KafkaContainerFixture.ResetAsync()` deletes topics outright. The shared host creates topics once at startup (`AutoProvision`) and never runs it again, so deleting them mid-run leaves listeners subscribed to removed partitions (`Unknown topic or partition`) and breaks every later round-trip. The shared base fixture exposes `ResetBrokerStateBetweenTests` (default `true`); the Kafka fixture sets it to `false`, so only PostgreSQL/MongoDB state is reset and the Kafka topics survive the whole collection. Each round-trip publishes a fresh envelope and waits for its own consumption via `TrackActivity`, so the tests never assert on leftover messages.
