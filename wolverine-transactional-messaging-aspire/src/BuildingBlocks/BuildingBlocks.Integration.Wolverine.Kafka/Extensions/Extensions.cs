...
public static class Extensions
{
    ...
    )
    {
        builder.AddWolverineMessaging(
            ...
            {
                var transport = !string.IsNullOrWhiteSpace(busOptions.ConnectionString)
                    ? options.UseKafka(busOptions.ConnectionString)
                    : options.UseKafkaUsingNamedConnection(busOptions.ConnectionName);
